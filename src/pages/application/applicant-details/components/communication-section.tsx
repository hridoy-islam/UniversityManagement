...
export default function CommunicationPage({ student }: CommunicationPageProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [logsRes, configsRes, draftsRes] = await Promise.all([
        axiosInstance.get("/email?limit=all", { params: { userId: student?._id, applicationId: appId, emailType: 'normal' } }),
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Compose and send an email to {student?.email}
              </DialogDescription>
            </DialogHeader>
            <Form {...form}>
