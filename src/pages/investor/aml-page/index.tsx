...
const InvestorAMLPage: React.FC<AMLProps> = () => {
  const { id } = useParams();
  ...
  const [originalDocuments, setOriginalDocuments] = useState<AMLFile>({
    image: '',
    passport: [],
    proofOfAddress: []
  });
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setOriginalDocuments(fetchedDocuments);
      } catch (error) {
  ...
  const saveToServer = async (updatedDocuments: AMLFile) => {
    setSaving(true);
    try {
      await axiosInstance.patch(`/users/${id}`, updatedDocuments);
    } catch (error) {
                    ...
                    const hasFiles =
                      id === 'image'
                        ...
                        : Array.isArray(documents[id as keyof AMLFile]) &&
                          (documents[id as keyof AMLFile] as string[]).length >
                            0;
...
export default InvestorAMLPage;
